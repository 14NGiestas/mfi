...
pure subroutine mfi_sgemm(a, b, c, transa, transb, alpha, beta)
    ...
    if (MFI_USE_CUBLAS == 1) then
        ...
        call f77_gemm(local_transa,local_transb,m,n,k, &
...
pure subroutine mfi_chemm(a, b, c, side, uplo, alpha, beta)
    ...
    if (MFI_USE_CUBLAS == 1) then
        ...
        call cublas_set_matrix(size(b,1),size(b,2),wp,    &
                            b,       max(1,size(b,1)), &
                            device_b,max(1,size(b,1)))
        call f77_hemm(local_side,local_uplo,m,n,local_alpha,device_a,lda,device_b, &
...
pure subroutine mfi_ssymm(a, b, c, side, uplo, alpha, beta)
    ...
    if (MFI_USE_CUBLAS == 1) then
        ...
        call cublas_set_matrix(size(b,1),size(b,2),wp,    &
                            b,       max(1,size(b,1)), &
                            device_b,max(1,size(b,1)))
        call f77_symm(local_side,local_uplo,m,n,local_alpha,device_a,lda,device_b, &
            ldb,local_beta,device_c,ldc)
        call cublas_get_matrix(size(c,1),size(c,2),wp,    &
                            device_c,max(1,size(c,1)), &
                            c,       max(1,size(c,1)))
