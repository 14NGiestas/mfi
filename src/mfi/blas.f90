...
pure subroutine mfi_sgemm(a, b, c, transa, transb, alpha, beta, use_gpu)
    ...
    if (local_use_gpu) then; block
        ...
        type(c_ptr) :: device_b
        type(c_ptr) :: device_c
        call cublas_alloc(size(a),wp,device_a)
        call cublas_alloc(size(b),wp,device_b)
        call cublas_alloc(size(c),wp,device_c)
        call cublas_set_matrix(size(a,1),size(a,2),wp, &
                            a,       max(1,size(a,1)), &
                            device_a,max(1,size(a,1)))
        call cublas_set_matrix(size(b,1),size(b,2),wp, &
                            b,       max(1,size(b,1)), &
                            device_b,max(1,size(b,1)))
        call cublas_set_matrix(size(c,1),size(c,2),wp, &
                            c,       max(1,size(c,1)), &
                            device_c,max(1,size(c,1)))
        call cublas_sgemm(local_transa,local_transb,m,n,k, &
                 local_alpha,device_a,lda,device_b,ldb,local_beta,device_c,ldc)
        call cublas_get_matrix(size(c,1),size(c,2),wp, &
                            device_c,max(1,size(c,1)), &
                            c,       max(1,size(c,1)))
        call cublas_free(device_a)
        call cublas_free(device_b)
        call cublas_free(device_c)
        return
    end block; end if
...
pure subroutine mfi_dgemm(a, b, c, transa, transb, alpha, beta, use_gpu)
    ...
    if (local_use_gpu) then; block
        ...
        call cublas_set_matrix(size(c,1),size(c,2),wp, &
                            c,       max(1,size(c,1)), &
                            device_c,max(1,size(c,1)))
        call cublas_dgemm(local_transa,local_transb,m,n,k, &
                 local_alpha,device_a,lda,device_b,ldb,local_beta,device_c,ldc)
        call cublas_get_matrix(size(c,1),size(c,2),wp, &
                            device_c,max(1,size(c,1)), &
                            c,       max(1,size(c,1)))
...
pure subroutine mfi_cgemm(a, b, c, transa, transb, alpha, beta, use_gpu)
    ...
    if (local_use_gpu) then; block
        ...
        call cublas_set_matrix(size(c,1),size(c,2),wp, &
                            c,       max(1,size(c,1)), &
                            device_c,max(1,size(c,1)))
        call cublas_cgemm(local_transa,local_transb,m,n,k, &
                 local_alpha,device_a,lda,device_b,ldb,local_beta,device_c,ldc)
        call cublas_get_matrix(size(c,1),size(c,2),wp, &
                            device_c,max(1,size(c,1)), &
                            c,       max(1,size(c,1)))
...
pure subroutine mfi_zgemm(a, b, c, transa, transb, alpha, beta, use_gpu)
    ...
    if (local_use_gpu) then; block
        ...
        call cublas_set_matrix(size(c,1),size(c,2),wp, &
                            c,       max(1,size(c,1)), &
                            device_c,max(1,size(c,1)))
        call cublas_zgemm(local_transa,local_transb,m,n,k, &
                 local_alpha,device_a,lda,device_b,ldb,local_beta,device_c,ldc)
        call cublas_get_matrix(size(c,1),size(c,2),wp, &
                            device_c,max(1,size(c,1)), &
                            c,       max(1,size(c,1)))
